...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            pi, logp_a = self.pi.forward(state, act)
            pi_list = list(pi.probs)
        ...
        if rand <= p_0: my_action = 0
        elif p_0 < rand <= p_1: my_action = 1
        elif p_1 < rand <= p_2: my_action = 2
        else: my_action = 3

        return my_action, v, logp_a[my_action]
...
class VPGBuffer:
    ...
    def get(self):
        ...
        self.tdres_buf = self.tdres_buf/(self.tdres_buf.sum())
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            log_pi = [self.ac.pi.forward(data['obs'][i],data['act'][i])[1] for i in range(len(data['act']))]
            phi = data['ret']

            E = np.dot(np.array(phi),np.array(log_pi))
            loss = -E/len(ep_returns)
            ...
            for _ in range(0):
                ...
                my_v = [self.ac.v.forward(data['obs'][i]) for i in range(len(data['obs']))]
                my_ret = data['ret']

                loss = torch.mean((torch.tensor(my_v, requires_grad=True) - my_ret)**2)
    ...
    def get_action(self, obs):
        ...
        pi, _ = self.ac.pi.forward(torch.as_tensor(obs, dtype=torch.float32))
        pi_list = list(pi.probs)
